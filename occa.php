...
<body>
    ...
    <div class="container">
        ...
        <main class="main">
            <form action="product.php" method="GET" id="survey-form" class="survey" novalidate>
                <div class="progressbar" tabindex="0" role="progressbar" aria-valuemin="1" aria-valuemax="5"
                    aria-valuenow="3" aria-valuetext="Question 1 of 5: How long have you been using our product?">
                ...
                <section class="survey__panel survey__panel--current" aria-hidden="false" data-index="1"
                    ...
                    <h2 class="survey__panel__question">
                        <span class="visuallyhidden">Question 3 of 5 </span>SELECT An
                        Occassion
                    </h2>
                    <div class="survey__panel__period">
                        <?php
                        $re=mysqli_query($con,"SELECT * FROM `occa` WHERE `occa`.`relation_id`='8'");
                        while($row=mysqli_fetch_assoc($re)){
                            ?>
                        <div class="form-group">
                            <input id="recipient" type="radio" value="<?php echo $row['id'] ?>" />
                            <label for="never"><?php echo $row['occa'] ?></label>
                        </div>
                        <?php
                        }
                        ?>
